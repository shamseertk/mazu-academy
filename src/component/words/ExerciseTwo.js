...
class ExerciseTwo extends React.Component{
  ...
  initiateValues = () => {
    ...
    const originalAvailWords = JSON.parse(JSON.stringify(allWords));
  ...
  handleFilterChange = (evt) => {
    const selectedLetter = evt.target.value;
    const checkArrLetter = selectedLetter === 'ا'
      ? ['أ', 'إ'] : [selectedLetter];
    
    const newAvailWords = selectedLetter === 'All'
      ? JSON.parse(JSON.stringify(allWords))
      : allWords.filter(aw => checkArrLetter.includes(aw.arabic.split('')[0]));
    const newOrgAvailWords = selectedLetter === 'All'
      ? JSON.parse(JSON.stringify(allWords))
      : allWords.filter(aw => checkArrLetter.includes(aw.arabic.split('')[0]));
    const maxQuestions = newAvailWords.length;
    const questionDetailObj = this.generateQuestionDetails(newAvailWords, newOrgAvailWords);
    this.setState({
      selectedLetter,
      maxQuestions,
      availableWords: newAvailWords,
      originalAvailWords: newOrgAvailWords,
      questionNumber: 1,
      ...questionDetailObj,
    });
  }
    ...
    return <React.Fragment>
      ...
      <TopPanel
        ...
        <Select
          ...
          {arabicWords && arabicWords.map(aWrd =>
            <MenuItem key={aWrd.arabic} value={aWrd.arabic}>{aWrd.arabic}</MenuItem>
          )}
        </Select>
      </TopPanel>
      <Question
